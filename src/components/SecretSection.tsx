...
export function SecretSection({ title, message, quizQuestion, quizAnswer, lang }: SecretSectionProps) {
            ...
            {!isOpen && !showQuiz && (
              ...
              >
                ...
                <span className="relative z-10 mt-20 text-[11px] font-mono tracking-[0.15em] text-on-surface-variant/80 uppercase group-hover:text-primary transition-colors">
                  {lang === "bn" ? "তোমার জন্য একটি কথা (খোল)" : "A Secret For You (Click to open)"}
                </span>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="flex flex-col space-y-3">
                  <input
                    type="text"
                    required
                    placeholder={lang === "bn" ? "উত্তর দিন..." : "Your answer here..."}
                    value={quizInput}
                    ...
                    className={`border rounded-sm py-2 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-primary text-center tracking-wide font-sans ${
                      quizError ? "border-red-500 bg-red-50 animate-shake" : "border-stone-300"
                    }`}
                  />
                  
                  {quizError && (
                    <p className="text-[10px] text-red-500 font-mono tracking-wider text-center">
                      ❌ {lang === "bn" ? "উত্তর হয়নি, আবার চেষ্টা করো" : "Incorrect response. Try again!"}
                    </p>
                  )}
            ...
            {isOpen && (
              ...
              >
                ...
                <div className="relative z-10">
                  <p
                    style={{ fontFamily: '"EB Garamond", serif' }}
                    className="text-lg md:text-xl text-on-background items-center italic leading-relaxed text-justify px-2 py-4 border-b border-primary/10"
                  >
                    {message}
                  </p>
                </div>
